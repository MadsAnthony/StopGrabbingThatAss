...
public class DavidArm : MonoBehaviour {
	...
	void OnMouseDown()
	{
		if (!animator.GetBool ("isHandCutOff")) {
			...
			//hand.GetComponent<DavidHandScript> ().handColor = GetComponent<SpriteRenderer>().color;
			hand.GetComponent<DavidHandScript> ().direction = theScale.x/transform.localScale.x;
			hand.GetComponent<DavidHandScript> ().gravity = -15;
